...
export async function hydrateDeploySecrets() {
  ...
  const copiedPassword = String(data.ssh_password || '');
  const password = copiedPassword.endsWith('*.') ? copiedPassword.slice(0, -1) : copiedPassword;
  ...
  process.env.HOSTINGER_SSH_PASSWORD = password;
  ...
  process.env.HOSTINGER_FTP_PASSWORD = password;
