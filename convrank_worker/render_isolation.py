...
async def render_and_axe_in_subprocess(url: str, screenshot: bool) -> dict[str, Any]:
    ...
    The parent GCL worker never owns Playwright/Chromium directly. Even if the
    Playwright driver or Chromium descendants ignore cancellation, the entire
    child process group is killed before the worker proceeds to Lighthouse.
    """
    ...
    try:
        ...
        try:
            stdout, stderr = await asyncio.wait_for(
                proc.communicate(payload), timeout=RENDER_CHILD_BUDGET_SECONDS
            )
        ...
        # Also kill a surviving descendant group after a normal child exit. The
        # group can outlive its leader if Chromium was orphaned during teardown.
        await _kill_process_group(proc)
