...
async function ai63Mount(){
  ...
  const reportPromise=ai63Report(token).then(data=>({data,error:null})).catch(error=>({data:null,error}));
  const host=await ai63WaitForHost();
  if(!host||document.getElementById('gcl-ai-analyst'))return;
  const settled=await reportPromise;
  if(settled.error){console.warn('ai-analyst-v63',settled.error.message);return}
  const d=settled.data;const ai=d?.ai_analyst;if(!d?.found||!ai?.available)return;
  if(document.getElementById('gcl-ai-analyst'))return;
