...
let queued=false;
function patch(){queued=false;if(!document.body)return;const w=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);const nodes=[];let n;while((n=w.nextNode()))nodes.push(n);for(const node of nodes){let v=node.nodeValue||'';let x=v;for(const [a,b] of REPLACEMENTS)if(x.includes(a))x=x.split(a).join(b);if(x!==v)node.nodeValue=x}
 if(/^\/ranking-site\/?$/.test(location.pathname)){const b=document.querySelector('.s3-brand b');const s=document.querySelector('.s3-brand small');if(b)b.textContent='GCL';if(s)s.textContent='GLOBAL CONVERSION LEAGUE';}
}
