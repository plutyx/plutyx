-- GCL v83: create a new immutable AI contract so existing GCL-AI-2.0
-- results remain auditable while new paid scans use the resilient free route.
...
begin
  ...
  values(p_audit_run_id,'canary',v_hash,'GCL-AI-2.1','nex-agi/nex-n2.5-mini:free','queued',now())
...
begin
  ...
  loop
    ...
    insert into sac.ai_enrichment_jobs(audit_run_id,purpose,evidence_hash,prompt_version,model_requested,status,updated_at)
    values(r.audit_run_id,'paid_analysis',v_hash,'GCL-AI-2.1','nex-agi/nex-n2.5-mini:free','queued',now())
