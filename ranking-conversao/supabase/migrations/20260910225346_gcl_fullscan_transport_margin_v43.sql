...
declare
  ...
  v_batch int:=0;
begin
  if not pg_try_advisory_xact_lock(hashtext('gcl_fullscan_dispatch_v43')) then return 0; end if;
  ...
  loop
    ...
    if j.mode in('lighthouse_full','full_paid','benchmark_deepening') then
      ...
      if j.mode='benchmark_deepening' then
        ...
        -- Keep a transport margin above the worker's bounded render + Lighthouse phases.
        -- The previous 120s ceiling caused valid heavy pages to be retried while the worker
        -- was still finishing, amplifying memory pressure on the single-instance worker.
        v_timeout:=150000;
        v_max_pages:=10;
      end if;
    else
