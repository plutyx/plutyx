...
begin
  ...
  return jsonb_build_object('checkout_ready',ready,'provider_environment',p.provider_environment,'url',case when ready then sac.checkout_url_for(p.product_code,p_intent_token) else null end);
end
$function$;
...
as $function$
declare
  v_url text:=btrim(p_url); v_bucket text:=left(coalesce(nullif(btrim(p_bucket_key),''),'anonymous'),128); v_domain text; v record; v_product sac.commercial_products%rowtype; v_source sac.benchmark_sources%rowtype; v_checkout_ready boolean:=false;
begin
  ...
  v_checkout_ready:=v_product.status='active' and v_product.provider_environment='live' and v_product.checkout_url is not null;
  ...
  if v.id is null then
    insert into sac.analysis_checkout_intents(url,normalized_domain,bucket_key,product_code,price_snapshot,currency) values(v_url,v_domain,v_bucket,'sac_analysis_2026',v_product.price,v_product.currency) returning * into v;
  end if;
  return jsonb_build_object(
    'intent_token',v.public_token,'status',v.status,'url',v.url,'domain',v.normalized_domain,'expires_at',v.expires_at,
    'offer',jsonb_build_object('product_code',v_product.product_code,'name',v_product.name,'price',v_product.price,'currency',v_product.currency,'checkout_url',case when v_checkout_ready then v_product.checkout_url else null end,'checkout_ready',v_checkout_ready,'provider_environment',v_product.provider_environment),
...
as $function$
with p as (
 select *, (status='active' and provider_environment='live' and checkout_url is not null) as live_ready,
   (status in ('draft','active') and provider_environment='test' and checkout_url is not null) as sandbox_ready
 from sac.commercial_products
), obj as (
 select product_code,jsonb_build_object('product_code',product_code,'name',name,'price',price,'currency',currency,'billing_interval',billing_interval,'checkout_url',case when live_ready then checkout_url else null end,'checkout_ready',live_ready,'sandbox_ready',sandbox_ready,'provider_environment',provider_environment,'grants',grants) j from p
), vals as (
 select (select price from p where product_code='sac_complete_entry_2026') entry_price,
...
set search_path to 'public', 'sac'
as $function$
declare v_base jsonb; v_external jsonb; v_commercial jsonb; v_payments jsonb; v_machine boolean; v_external_ok boolean; v_live_staged int:=0;
begin
  ...
  select count(*)::int into v_live_staged from sac.commercial_provider_configs
  where provider='stripe' and provider_environment='live' and status='paused' and provider_product_id is not null and provider_price_id is not null and provider_payment_link_id is null and checkout_url is null;
  v_payments := coalesce(v_base->'payments','{}'::jsonb) || jsonb_build_object('live_staged_products',v_live_staged,'live_catalog_staged',v_live_staged=6,'live_catalog_public_checkout_open',coalesce((v_base->'payments'->>'live_ready')::boolean,false));
  v_base := jsonb_set(v_base,'{payments}',v_payments,true);
  v_commercial := (v_base->'commercial_go_live') || jsonb_build_object('external_controls',v_external,'external_attestations_passed',v_external_ok,'external_blockers',v_external->'blockers','commercial_ready',v_machine and v_external_ok);
