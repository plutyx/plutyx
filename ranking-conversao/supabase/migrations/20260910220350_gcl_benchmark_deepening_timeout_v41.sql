...
declare
  ...
  v_batch int:=0;
begin
  if not pg_try_advisory_xact_lock(hashtext('gcl_fullscan_dispatch_v41')) then return 0; end if;
  ...
  for j in
    select id,url,mode,attempts
    from sac.fullscan_jobs
    where status='queued'
    order by case when mode='full_paid' then 0 when mode='lighthouse_full' then 1 when mode='benchmark_deepening' then 9 else 2 end,created_at
  ...
  loop
    ...
    if j.mode in('lighthouse_full','full_paid','benchmark_deepening') then
      ...
      if j.mode='benchmark_deepening' then
        -- The heavy worker performs render+axe and then Lighthouse sequentially.
        -- Lighthouse has its own 90s ceiling, so the transport must exceed that wall-clock budget.
        v_timeout:=155000;
        v_max_pages:=1;
      else
        v_timeout:=120000;
    ...
    update sac.fullscan_jobs
      set status='processing',attempts=attempts+1,started_at=coalesce(started_at,now()),updated_at=now(),error=null,
          response=case when mode='benchmark_deepening' then coalesce(response,'{}'::jsonb)||jsonb_build_object(
            'dispatch_profile',case when j.attempts>=1 then 'background_root_retry_155s' else 'background_root_155s' end,
            'requested_max_pages',v_max_pages,
            'requested_timeout_ms',v_timeout,
            'pipeline_note','render+axe then lighthouse; transport budget exceeds lighthouse 90s ceiling'
          ) else response end
    where id=j.id;
...
revoke all on function sac.dispatch_fullscan_jobs(integer) from public,anon,authenticated;
grant execute on function sac.dispatch_fullscan_jobs(integer) to service_role,postgres;