...
declare
  ...
  v_batch int:=0;
begin
  if not pg_try_advisory_xact_lock(hashtext('gcl_fullscan_dispatch_v41_1')) then return 0; end if;
  ...
  loop
    ...
    if j.mode in('lighthouse_full','full_paid','benchmark_deepening') then
      ...
      if j.mode='benchmark_deepening' then
        -- Background deepening may incur a free-instance cold start before the sequential
        -- render+axe + Lighthouse pipeline. Keep this larger budget isolated from paid scans.
        v_timeout:=210000;
        v_max_pages:=1;
      else
        v_timeout:=120000;
