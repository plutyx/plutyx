...
begin
  ...
  values(p_audit_run_id,'canary',v_hash,'GCL-AI-2.0','nex-agi/nex-n2.5-mini:free','queued',now())
  on conflict(audit_run_id,evidence_hash,prompt_version)
  do update set model_requested=excluded.model_requested,status=case when sac.ai_enrichment_jobs.status='completed' then 'completed' else 'queued' end,error=null,completed_at=case when sac.ai_enrichment_jobs.status='completed' then sac.ai_enrichment_jobs.completed_at else null end,updated_at=now()
...
begin
  ...
  loop
    ...
    insert into sac.ai_enrichment_jobs(audit_run_id,purpose,evidence_hash,prompt_version,model_requested,status,updated_at)
    values(r.audit_run_id,'paid_analysis',v_hash,'GCL-AI-2.0','nex-agi/nex-n2.5-mini:free','queued',now())
