...
async function demandPlan(businessId:number,historyDays:number,coverDays:number){
 ...
 const completedIds=(completed||[]).map((row:any)=>Number(row.id))
 const openIds=(openOrders||[]).map((row:any)=>Number(row.id))
 const orderIds=[...new Set([...completedIds,...openIds])]
 let orderItems:any[]=[]
 if(orderIds.length){
  const result=await admin.from('order_items').select('order_id,product_id,quantity').eq('business_id',businessId).in('order_id',orderIds)
 ...
 if(productIds.length){
  const result=await admin.from('recipe_items').select('product_id,ingredient_id,qty_used_milliunits').in('product_id',productIds)
  if(result.error)throw result.error
  recipeRows=result.data||[]
 }

 const completedMap=new Map((completed||[]).map((row:any)=>[Number(row.id),row]))
 const openSet=new Set(openIds)
 const recipeMap=new Map<number,any[]>()
 for(const row of recipeRows){
  const pid=Number(row.product_id),rows=recipeMap.get(pid)||[]
  rows.push(row);recipeMap.set(pid,rows)
 }
 ...
 for(const row of orderItems){
  const orderId=Number(row.order_id),productId=Number(row.product_id),quantity=Math.max(0,Number(row.quantity||0))
  if(!productId||quantity<=0)continue
  observedUnits+=quantity
  if((recipeMap.get(productId)||[]).length)mappedUnits+=quantity
  if(openSet.has(orderId)){openQty.set(productId,(openQty.get(productId)||0)+quantity);continue}
  const order:any=completedMap.get(orderId)
  if(!order)continue
  const ts=new Date(order.completed_at||order.created_at||0).getTime(),stats=productStats.get(productId)||{recent:0,prior:0}
  if(Number.isFinite(ts)&&ts>=recentCutoff)stats.recent+=quantity
  else stats.prior+=quantity
  productStats.set(productId,stats)
 }

 const completedCount=(completed||[]).length
 const confidence=completedCount>=20?'high':completedCount>=8?'medium':'low'
 ...
 for(const productId of productIds){
  ...
  const daily=stats.recent>0&&stats.prior>0?0.65*recentDaily+0.35*priorDaily:stats.recent>0?recentDaily:priorDaily
 ...
 for(const productId of productIds){
  ...
  for(const recipe of recipes){
   const ingredientId=Number(recipe.ingredient_id),perUnit=Math.max(0,Number(recipe.qty_used_milliunits||0))
   if(!ingredientId||perUnit<=0)continue
   const current=demandByIngredient.get(ingredientId)||{committed:0,forecast:0}
   current.committed+=committedUnits*perUnit
   current.forecast+=forecastUnits*perUnit
   demandByIngredient.set(ingredientId,current)
  }
 }
 ...
 const rows=(ingredients||[]).map((row:any)=>{
  ...
  const referenceQty=Math.max(0,Number(row.last_purchase_qty_milliunits||0)),referencePrice=Math.max(0,Number(row.last_purchase_price_cents||0))
  ...
  return{
   ingredient_id:Number(row.id),name:row.name,unit:row.unit,on_hand_milliunits:onHand,par_level_milliunits:minimum,reorder_target_milliunits:target,
   baseline_purchase_milliunits:Math.round(baseline),committed_milliunits:Math.round(demand.committed),forecast_milliunits:Math.round(demand.forecast),projected_after_committed_milliunits:Math.round(projectedAfterCommitted),demand_uplift_milliunits:Math.round(uplift),stock_cover_days:cover===null?null:Math.round(cover*10)/10,
   suggested_purchase_milliunits:Math.round(suggested),last_purchase_price_cents:referencePrice,last_purchase_qty_milliunits:referenceQty,estimated_landed_cents:estimated,cost_confidence:referencePrice>0&&referenceQty>0?'reference':'missing',demand_signal:demand.forecast>0?confidence:'none',reason,tone,
  }
 }).filter((row:any)=>row.suggested_purchase_milliunits>0)
 rows.sort((a:any,b:any)=>{
  const rank:any={critical:3,warning:2,attention:1,stable:0}
  return rank[b.tone]-rank[a.tone]||Number(b.demand_uplift_milliunits)-Number(a.demand_uplift_milliunits)||b.suggested_purchase_milliunits-a.suggested_purchase_milliunits||String(a.name).localeCompare(String(b.name),'pt-BR')
 })
 return{
...
async function history(businessId:number,limit:number){
 ...
 const rows=(purchases||[]).map((row:any)=>{
  const ingredient:any=ingredientMap.get(Number(row.ingredient_id))||{},supplier:any=supplierMap.get(Number(row.supplier_id))||{},quantity=Math.max(1,Number(row.quantity_milliunits||0)),landed=money(row.total_cents)+money(row.freight_cents)+money(row.tax_cents)
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(path==='/readyz'&&method==='GET'){
   const[{error:purchasesError},{error:ordersError},{error:recipesError}]=await Promise.all([
    admin.from('purchases').select('id',{head:true,count:'exact'}),admin.from('orders').select('id',{head:true,count:'exact'}),admin.from('recipe_items').select('id',{head:true,count:'exact'}),
   ])
   return purchasesError||ordersError||recipesError?fail('database_not_ready',503):json({ok:true,database:'ready',demand_inputs:'ready',version:'5.3.0'})
  }
  const auth=await authUser(req)
  if(!auth)return fail('Sessão inválida',401)
  const profile:any=await ensureProfile(auth)
  let match=path.match(/^\/businesses\/(\d+)\/purchase-plan$/)
  if(match&&method==='GET'){
   const businessId=Number(match[1])
   if(!await member(profile.id,businessId))return fail('Sem acesso',403)
  ...
  if(match&&method==='POST'){
   const businessId=Number(match[1])
   if(!await member(profile.id,businessId,['owner','admin']))return fail('Somente owner/admin',403)
   ...
   if(supplierId){
    ...
    if(error)throw error
    if(!supplier)return fail('Fornecedor não pertence a esta operação',422)
  ...
  console.error('Compras 360 v5.3 error',error)
