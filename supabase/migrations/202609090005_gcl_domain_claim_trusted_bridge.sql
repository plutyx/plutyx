-- GCL production hardening: allow the trusted Edge verifier to persist a claim result
-- using service_role. Direct authenticated execution is revoked in the next migration
-- only after the Edge function has been promoted.
...
begin
  -- Browser callers still use auth.uid() during the bridge rollout. The trusted
  -- Edge verifier runs with service_role and may derive the challenge owner only
  -- after the claim token has already been resolved by the authenticated context RPC.
  ...
  if p_verified then
    ...
    update sac.domains
    set claimed=true,
        claim_method=c.method,
        owner_user_id=coalesce(owner_user_id,uid),
        updated_at=now()
    where id=d.id;
  end if;
