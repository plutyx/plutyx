...
def create_token(user_id: int, email: str) -> str:
    ...
    payload={"sub":str(user_id),"email":email,"iat":int(now.timestamp()),"exp":int((now+timedelta(minutes=settings.access_token_minutes)).timestamp()),"iss":"cozinha360"}
...
async def current_user(credentials: Annotated[HTTPAuthorizationCredentials|None,Depends(bearer)], db: Annotated[Session,Depends(get_db)]):
    ...
    payload=decode_token(credentials.credentials); user=db.get(User,int(payload["sub"]))
