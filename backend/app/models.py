...
class Ingredient(Base, TS):
    ...
    last_purchase_price_cents: Mapped[int]=mapped_column(Integer,default=0)
    last_purchase_qty_milliunits: Mapped[int]=mapped_column(Integer,default=1000)
    usable_qty_milliunits: Mapped[int]=mapped_column(Integer,default=1000)
    on_hand_milliunits: Mapped[int]=mapped_column(Integer,default=0)
    par_level_milliunits: Mapped[int]=mapped_column(Integer,default=0)
    reorder_target_milliunits: Mapped[int]=mapped_column(Integer,default=0)
    soft_deleted: Mapped[bool]=mapped_column(Boolean,default=False)
...
class Supplier(Base, TS):
    ...
    backup_supplier: Mapped[bool]=mapped_column(Boolean,default=False)
...
class Purchase(Base, TS):
    ...
    quantity_milliunits: Mapped[int]=mapped_column(Integer)
    total_cents: Mapped[int]=mapped_column(Integer)
    freight_cents: Mapped[int]=mapped_column(Integer,default=0)
...
class Product(Base, TS):
    ...
    units_per_batch: Mapped[int]=mapped_column(Integer,default=1)
    packaging_cents_per_unit: Mapped[int]=mapped_column(Integer,default=0)
    energy_cents_per_batch: Mapped[int]=mapped_column(Integer,default=0)
    labor_cents_per_batch: Mapped[int]=mapped_column(Integer,default=0)
    soft_deleted: Mapped[bool]=mapped_column(Boolean,default=False)
    version: Mapped[int]=mapped_column(Integer,default=1)
...
class RecipeItem(Base, TS):
    ...
    ingredient_id: Mapped[int]=mapped_column(ForeignKey("ingredients.id",ondelete="RESTRICT"),index=True)
    qty_used_milliunits: Mapped[int]=mapped_column(Integer)
...
class Channel(Base, TS):
    ...
    promo_cents: Mapped[int]=mapped_column(Integer,default=0)
    media_cents: Mapped[int]=mapped_column(Integer,default=0)
...
class ProductChannelPrice(Base, TS):
    ...
    desired_contribution_cents: Mapped[int]=mapped_column(Integer,default=1000)
...
class Order(Base, TS):
    ...
    variable_cost_cents: Mapped[int]=mapped_column(Integer,default=0)
    contribution_cents: Mapped[int]=mapped_column(Integer,default=0)
...
class OrderItem(Base, TS):
    __tablename__="order_items"; __table_args__=(Index("ix_order_items_order","order_id"),Index("ix_order_items_product","product_id"))
    id: Mapped[int]=mapped_column(primary_key=True)
    business_id: Mapped[int]=mapped_column(ForeignKey("businesses.id",ondelete="CASCADE"),index=True)
    ...
    unit_variable_cost_cents: Mapped[int]=mapped_column(Integer,default=0)


class ProductionBatch(Base, TS):
    __tablename__="production_batches"; __table_args__=(Index("ix_production_business_status","business_id","status"),)
    id: Mapped[int]=mapped_column(primary_key=True)
    business_id: Mapped[int]=mapped_column(ForeignKey("businesses.id",ondelete="CASCADE"),index=True)
    product_id: Mapped[int]=mapped_column(ForeignKey("products.id",ondelete="RESTRICT"),index=True)
    responsible_user_id: Mapped[int|None]=mapped_column(ForeignKey("users.id",ondelete="SET NULL"),nullable=True,index=True)
...
class Loss(Base, TS):
    ...
    qty_milliunits: Mapped[int]=mapped_column(Integer,default=0)
...
class CapacityStep(Base, TS):
    ...
    business_id: Mapped[int]=mapped_column(ForeignKey("businesses.id",ondelete="CASCADE"),index=True)
    name: Mapped[str]=mapped_column(String(120))
    minutes_per_order: Mapped[int]=mapped_column(Integer)
...
class OutboxEvent(Base):
    ...
    id: Mapped[int]=mapped_column(primary_key=True)
    business_id: Mapped[int|None]=mapped_column(ForeignKey("businesses.id",ondelete="CASCADE"),nullable=True,index=True)
