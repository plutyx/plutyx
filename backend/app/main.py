...
class ProductIn(BaseModel): name:str; category:str=""; active:bool=True; units_per_batch:int=Field(gt=0); packaging_cents_per_unit:int=Field(ge=0); energy_cents_per_batch:int=Field(ge=0,default=0); labor_cents_per_batch:int=Field(ge=0,default=0)
...
@app.get("/health")
def health(): return {"ok":True,"service":"cozinha360-api","version":"0.6.0","environment":settings.environment}
...
def list_ingredients(business_id:int,user:Annotated[User,Depends(current_user)],db:Annotated[Session,Depends(get_db)]):
    member_or_404(db,user,business_id); rows=db.scalars(select(Ingredient).where(Ingredient.business_id==business_id,Ingredient.soft_deleted==False).order_by(Ingredient.name)).all(); return [{"id":x.id,"name":x.name,"unit":x.unit,"last_purchase_price_cents":x.last_purchase_price_cents,"usable_qty_milliunits":x.usable_qty_milliunits,"on_hand_milliunits":x.on_hand_milliunits,"par_level_milliunits":x.par_level_milliunits,"reorder_target_milliunits":x.reorder_target_milliunits,"version":x.version} for x in rows]
...
def add_purchase(business_id:int,data:PurchaseIn,user:Annotated[User,Depends(current_user)],db:Annotated[Session,Depends(get_db)]):
    ...
    new_price=round(landed*ing.last_purchase_qty_milliunits/data.quantity_milliunits) if data.quantity_milliunits else landed; ing.last_purchase_price_cents=new_price; ing.on_hand_milliunits+=data.quantity_milliunits; ing.version+=1
    ...
    alert=increase_bps>=threshold; db.flush(); audit(db,business_id,user.id,"purchase.created","purchase",str(p.id),{"ingredient_id":ing.id,"increase_bps":increase_bps,"alert":alert,"stock_after":ing.on_hand_milliunits}); db.commit()
    return {"id":p.id,"ingredient_id":ing.id,"old_reference_cents":old,"new_reference_cents":new_price,"increase_bps":increase_bps,"price_alert":alert,"on_hand_milliunits":ing.on_hand_milliunits,"idempotency_key":key}
...
def products(business_id:int,user:Annotated[User,Depends(current_user)],db:Annotated[Session,Depends(get_db)]):
    member_or_404(db,user,business_id); rows=db.scalars(select(Product).where(Product.business_id==business_id,Product.soft_deleted==False).order_by(Product.name)).all(); return [{"id":p.id,"name":p.name,"category":p.category,"active":p.active,"units_per_batch":p.units_per_batch,"packaging_cents_per_unit":p.packaging_cents_per_unit,"energy_cents_per_batch":p.energy_cents_per_batch,"labor_cents_per_batch":p.labor_cents_per_batch,"version":p.version} for p in rows]
...
def orders(business_id:int,user:Annotated[User,Depends(current_user)],db:Annotated[Session,Depends(get_db)]):
    member_or_404(db,user,business_id); rows=db.scalars(select(Order).where(Order.business_id==business_id).order_by(Order.created_at.desc()).limit(200)).all(); return [{"id":o.id,"status":o.status,"source":o.source,"total_cents":o.total_cents,"variable_cost_cents":o.variable_cost_cents,"contribution_cents":o.contribution_cents,"paid":o.paid,"delayed":o.delayed,"error_flag":o.error_flag,"inventory_consumed":o.inventory_consumed,"version":o.version,"created_at":o.created_at.isoformat()} for o in rows]
...
def update_order_status(business_id:int,order_id:int,data:OrderStatusIn,user:Annotated[User,Depends(current_user)],db:Annotated[Session,Depends(get_db)]):
    ...
    if data.status=="completed":
        inventory_result=consume_order_inventory(db,business_id,o.id)
    ...
    audit(db,business_id,user.id,"order.status_changed","order",str(o.id),{"status":data.status,"version":o.version,"inventory":inventory_result}); db.commit(); return {"id":o.id,"status":o.status,"version":o.version,"inventory":inventory_result}
...
def add_loss(business_id:int,data:LossIn,user:Annotated[User,Depends(current_user)],db:Annotated[Session,Depends(get_db)]):
    ...
    if data.ingredient_id:
        ...
        if not ing or ing.business_id!=business_id: raise HTTPException(404,"Ingrediente não encontrado")
        ing.on_hand_milliunits-=data.qty_milliunits; ing.version+=1
    x=Loss(business_id=business_id,**data.model_dump()); db.add(x); db.flush(); audit(db,business_id,user.id,"loss.created","loss",str(x.id),{"ingredient_id":data.ingredient_id,"qty_milliunits":data.qty_milliunits}); db.commit(); return {"id":x.id}
