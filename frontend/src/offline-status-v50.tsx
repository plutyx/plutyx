...
export function OfflineQueueStatus(){
 ...
 return <>{visible&&<button className={`offline50-pill ${online?'online':'offline'} ${failed?'has-failed':''}`} type="button" onClick={()=>setOpen(true)} aria-label="Status de conexão e pedidos offline">{online?<Wifi size={15}/>:<WifiOff size={15}/>}<span>{online?(items.length?`${items.length} para sincronizar`:'online'):'modo offline'}</span>{failed>0&&<b>{failed}</b>}</button>}
