...
function playDelivery(ctx:AudioContext){const t=ctx.currentTime+.03;tone(ctx,520,t,.12,.04);tone(ctx,520,t+.15,.12,.04);tone(ctx,680,t+.3,.18,.05)}
...
export function KitchenAlerts(){
  ...
  const ctxRef=useRef<AudioContext|null>(null),knownRef=useRef<Map<number,string>>(new Map()),businessRef=useRef<number>(0),timerRef=useRef<number|undefined>(undefined)

  async function resolveBusiness(){if(businessRef.current)return businessRef.current;const me=await request('/me',{},token);businessRef.current=Number(me.businesses?.[0]?.id||0);return businessRef.current}
  ...
  return <div className="kitchen-alert-wrap"><button className={`kitchen-alert-toggle ${enabled?'on':''}`} onClick={toggle} title="Alertas sonoros de pedidos"><span>{enabled?<Volume2 size={17}/>:<VolumeX size={17}/>}</span><b>{label}</b>{enabled&&<BellRing size={15}/>}</button>{error&&<small>{error}</small>}</div>
}
