...
import {
  ...
  Network,Plus,Route,Search,ShieldCheck,ShoppingBag,SlidersHorizontal,
...
const actions:Action[]=[
  ...
  {label:'Delivery OS',href:'/?delivery=1',group:'Operar',description:'marcas, áreas, entregadores e recompra do delivery',keywords:'delivery dark kitchen multimarca entregadores zonas taxas cupom fidelidade',icon:Route},
  ...
  return <>
    ...
    {open&&<div className="operator-launcher-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setOpen(false)}}>
      <section className="operator-launcher-panel" role="dialog" aria-modal="true" aria-label="Launcher do Cozinha 360">
        ...
        <label className="operator-launcher-search"><Search size={18}/><input ref={inputRef} value={query} onChange={event=>setQuery(event.target.value)} placeholder="Buscar caixa, delivery, CRM, cozinha…" aria-label="Buscar função"/><span>ESC</span></label>
