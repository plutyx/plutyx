...
import {
  ...
  Network,Plus,Scale,Search,ShieldCheck,ShoppingBag,SlidersHorizontal,Sparkles,Store,
  ...
  return <>
    ...
    {open&&<div className="operator-launcher-backdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setOpen(false)}}>
      <section className="operator-launcher-panel" role="dialog" aria-modal="true" aria-label="Launcher do Cozinha 360">
        ...
        <label className="operator-launcher-search"><Search size={18}/><input ref={inputRef} value={query} onChange={event=>setQuery(event.target.value)} placeholder="Buscar delivery, caixa, CMV, CRM, cozinha, rede…" aria-label="Buscar função"/><span>ESC</span></label>
