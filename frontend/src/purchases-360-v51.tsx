...
import{AlertTriangle,ArrowLeft,ArrowRight,CheckCircle2,CircleDollarSign,ClipboardCheck,History,PackageCheck,RefreshCcw,ShieldCheck,ShoppingBasket,TrendingDown,TrendingUp,Truck,WifiOff}from'lucide-react'
...
export function Purchases360Route(){
 ...
 const[ingredientId,setIngredientId]=useState(0),[quantity,setQuantity]=useState(''),[total,setTotal]=useState(''),[freight,setFreight]=useState('0'),[tax,setTax]=useState('0')
 ...
 async function load(target=businessId){
  ...
  try{
   ...
   if(!id){setPlan(null);setHistory(null);setIngredients([]);return}
   const[ingredientRows,planData,historyData]=await Promise.all([
   ...
   const active=(ingredientRows||[]) as Ingredient[];setIngredients(active);setPlan(planData as Plan);setHistory(historyData as HistoryData)
 ...
 async function submit(event:React.FormEvent){
  ...
  try{
   const result=await purchasesRequest(`/businesses/${businessId}/purchases`,{method:'POST',body:JSON.stringify({ingredient_id:selected.id,quantity_milliunits:q,total_cents:productCents,freight_cents:toCents(freight),tax_cents:toCents(tax),idempotency_key:crypto.randomUUID()})},token)
