...
export function Suppliers360Route(){
 ...
 async function load(target=businessId){
  ...
  try{
   ...
   const next=ingredientId&&active.some(row=>row.id===ingredientId)?ingredientId:Number(active[0]?.id||0);setIngredientId(next)
