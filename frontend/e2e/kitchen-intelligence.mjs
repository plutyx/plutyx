...
await page.route('**/cozinha360-kds-v54/businesses/1/kds/settings',route=>route.fulfill(json({business_id:1,default_kds_sla_minutes:20,products:[{id:10,name:'Wrap Intelligence',category:'wraps',prep_sla_minutes:15,active:true}],channels:[{id:3,name:'WhatsApp',order_sla_minutes:25,traffic_active:true}]})))
await page.route('**/cozinha360-kds-v54/businesses/1/kds',route=>route.fulfill(json({business_id:1,default_kds_sla_minutes:20,summary:{open_orders:1,prep_orders:1,overdue:0,at_risk:0,sla_conflicts:0},orders:[{id:42,status:'new',source:'whatsapp',total_cents:4000,age_minutes:8,delayed:false,version:1,prep_sla_minutes:15,channel_sla_minutes:25,effective_sla_minutes:15,due_in_minutes:7,urgency:'on_track',sla_conflict:false,channel:{id:3,name:'WhatsApp'},items:[{product_id:10,name:'Wrap Intelligence',quantity:2,prep_sla_minutes:15,sla_source:'product'}]}]})))
...
await page.route('**/api/businesses/1/products/10/recipe',route=>route.fulfill(json([{product_id:10,ingredient_id:7,qty_used_milliunits:200}])))
...
try{
  ...
  await productCard.getByText('faltam 7 min',{exact:true}).waitFor()
  ...
  console.log('kitchen intelligence component + SLA view ok')
}catch(error){
